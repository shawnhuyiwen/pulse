...
void CommonDataModelTest::BasicBlackBoxTest(const std::string& sOutputDirectory)
{
  ...
  BlackBox1.MapBlackBox<SEFluidBlackBox, SEFluidCircuitNode, SEFluidCircuitPath>(BB1Path1, BB1Path2);
  ...
  BlackBox2.MapBlackBox<SEFluidBlackBox, SEFluidCircuitNode, SEFluidCircuitPath>(BB2Path1, BB2Path2);
...
void CommonDataModelTest::SimpleBlackBoxTest(const std::string& sOutputDirectory)
{
  ...
  for (unsigned int iter = 0; iter < (1 << 8); ++iter)
  {
    ...
    if (!isCombined)
    {
      ...
      blackBoxPassive->MapBlackBox<SEFluidBlackBox, SEFluidCircuitNode, SEFluidCircuitPath>(pathBBPassiveSource, pathBBPassiveTarget);
      ...
      blackBoxDriver->MapBlackBox<SEFluidBlackBox, SEFluidCircuitNode, SEFluidCircuitPath>(pathBBDriverSource, pathBBDriverTarget);
...
void CommonDataModelTest::WindkesselBlackBoxTest(const std::string& sOutputDirectory)
{
  ...
  for (unsigned int iter = 0; iter < (1 << 8); ++iter)
  {
    ...
    if (!isCombined)
    {
      ...
      blackBoxDriver = managerBlackBox.GetLiquidBlackBox("blackBoxDriver", "");// jbw correct names
      blackBoxDriver->MapBlackBox<SEFluidBlackBox, SEFluidCircuitNode, SEFluidCircuitPath>(pathBlackBoxDriverSource, pathBlackBoxDriverTarget);
      ...
      blackBoxWindkessel->MapBlackBox<SEFluidBlackBox, SEFluidCircuitNode, SEFluidCircuitPath>(pathBlackBoxWindkesselSource, pathBlackBoxWindkesselTarget);
