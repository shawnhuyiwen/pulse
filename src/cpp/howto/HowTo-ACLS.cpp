...
void HowToACLS()
{
  ...
  {
    {eHeartRhythm::FineVentricularFibrillation, "fvfib"},
    //{eHeartRhythm::CoarseVentricularFibrillation, "cvfib"}
  };
  std::map<eHeartRhythm, std::string> endArrhythmias =
  {
    {eHeartRhythm::NormalSinus, "sinus"},
    {eHeartRhythm::SinusTachycardia, "tachy"},
    //{eHeartRhythm::SinusBradycardia, "brady"},
  ...
  for (auto start : startArrhythmias)
  {
    for (auto end : endArrhythmias)
    {
      for (auto ivn : interventions)
      {
        ...
        std::string name = start.second + "_to_" + end.second + "_with_" + ivn +"_intervention";
