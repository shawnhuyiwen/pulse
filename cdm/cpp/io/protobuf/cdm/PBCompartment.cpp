...
void PBCompartment::Serialize(const cdm::GasCompartmentData& src, SEGasCompartment& dst, SESubstanceManager& subMgr, SECircuitManager* circuits)
{
  ...
  if (src.substancequantity_size() > 0)
  {
    for (int i = 0; i < src.substancequantity_size(); i++)
    {
      ...
      PBCompartment::Load(d, dst.CreateSubstanceQuantity(*sub));
    }
  }
}
...
void PBCompartment::Serialize(const SEGasCompartment& src, cdm::GasCompartmentData& dst)
{
  ...
  for (SEGasSubstanceQuantity* subQ : src.m_SubstanceQuantities)
    dst.mutable_substancequantity()->AddAllocated(PBCompartment::Unload(*subQ));
}
...
void PBCompartment::Serialize(const cdm::SubstanceQuantityData& src, SESubstanceQuantity& dst)
{
  dst.Clear();
  // Name will be used to get substance for ctor
}
void PBCompartment::Serialize(const SESubstanceQuantity& src, cdm::SubstanceQuantityData& dst)
{
  dst.set_substance(src.m_Substance.GetName());
}

void PBCompartment::Load(const cdm::GasSubstanceQuantityData& src, SEGasSubstanceQuantity& dst)
{
  PBCompartment::Serialize(src, dst);
}
void PBCompartment::Serialize(const cdm::GasSubstanceQuantityData& src, SEGasSubstanceQuantity& dst)
{
  PBCompartment::Serialize(src.substancequantity(), dst);
...
cdm::GasSubstanceQuantityData* PBCompartment::Unload(const SEGasSubstanceQuantity& src)
{
  cdm::GasSubstanceQuantityData* dst = new cdm::GasSubstanceQuantityData();
  PBCompartment::Serialize(src, *dst);
  return dst;
}
void PBCompartment::Serialize(const SEGasSubstanceQuantity& src, cdm::GasSubstanceQuantityData& dst)
{
  PBCompartment::Serialize(src, *dst.mutable_substancequantity());
...
void PBCompartment::Load(const cdm::LiquidSubstanceQuantityData& src, SELiquidSubstanceQuantity& dst)
{
  PBCompartment::Serialize(src, dst);
}
void PBCompartment::Serialize(const cdm::LiquidSubstanceQuantityData& src, SELiquidSubstanceQuantity& dst)
{
  PBCompartment::Serialize(src.substancequantity(), dst);
  if (!dst.m_Compartment.HasChildren())
  {
...
cdm::LiquidSubstanceQuantityData* PBCompartment::Unload(const SELiquidSubstanceQuantity& src)
{
  cdm::LiquidSubstanceQuantityData* dst = new cdm::LiquidSubstanceQuantityData();
  PBCompartment::Serialize(src, *dst);
  return dst;
}
void PBCompartment::Serialize(const SELiquidSubstanceQuantity& src, cdm::LiquidSubstanceQuantityData& dst)
{
  PBCompartment::Serialize(src, *dst.mutable_substancequantity());
  // Even if you have children, I am unloading everything, this makes the output actually usefull...
